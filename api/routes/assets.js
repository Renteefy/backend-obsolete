...
router.post(
  "/",
  checkAuth,
  upload.fields([{ name: "AssetImage", maxCount: 1 }]),
  (req, res, next) => {
      ...
      if (doc) {
        ...
        alreadySentQuery.exec((err, doc) => {
          if (err) return err;
            ...
            res
              .status(200)
              .json({ assetResponse: assetResponse, notifiResponse: doc });
        });
      } else {
        res.status(404).json({ message: "No Valid Entry Found" });
      }
...
function findAlreadySent(username, assetTitle) {
  ...
  let query = Notification.find({ rentee: username, title: assetTitle }).select(
    "title status owner rentee assetID"
  );
